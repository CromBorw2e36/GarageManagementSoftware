...
namespace GarageManagementSoftware.Models.Service.Function.WorkFlow
{
    public class ProjectService : DataSystemCommon, IProject
    {
        ...
        public StatusMessege<ProjectModel> Delete(HttpRequest httpRequest, ProjectModel model)
        {
            ...
            {
                return new StatusMessege<ProjectModel>().setStatus(EnumMessage.ERROR.GetStatus()).setMessage(EnumMessage.NOT_CODE.GetMessage()).setData(model);
            }
            else
            {
                ...
                {
                    return new StatusMessege<ProjectModel>().setStatus(EnumMessage.ERROR.GetStatus()).setMessage(EnumMessage.NOT_FOUND_ITEM.GetMessage()).setData(model);
                }
            }
        }
        ...
        {
            var row = this._dataContext.ProjectModel.Where(item =>
        ...
        public StatusMessege<ProjectModel> Insert(HttpRequest httpRequest, ProjectModel model)
        {
            ...
            else
            {
                try
                {
                    ...
                    this._dataContext.ProjectModel.Add(model);
                    this._dataContext.SaveChanges();
                    return new StatusMessege<ProjectModel>().setStatus(EnumMessage.SUCCESS.GetStatus()).setMessage(EnumMessage.SUCCESS.GetMessage()).setData(model);
                }
                catch
                {
                    return new StatusMessege<ProjectModel>().setStatus(EnumMessage.ERROR.GetStatus()).setMessage(EnumMessage.ERROR.GetMessage()).setData(model);
                }
            }
        }
